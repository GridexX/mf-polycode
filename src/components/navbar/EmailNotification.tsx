  ...
  const handleResendEmail = () => {
    resendEmail(email.id)
      .then(() => {
        toastSuccess(
          <Typography>
            {i18n.t('components.navbar.modal.resendSuccess')}
          </Typography>
        );
      })
      .catch(() => {
        toastError(
          <Typography>
            {i18n.t('components.navbar.modal.resendError')}
          </Typography>
        );
      });
  };
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)}>
        ...
        >
          <Typography variant="h6" component="h2">
            {i18n.t('components.navbar.modal.title')}
          </Typography>
          <Typography sx={{ mt: 2 }}>
            {i18n.t('components.navbar.modal.sentMail')} {email.email}{' '}
            {i18n.t('components.navbar.modal.sentMailSuite')}
          </Typography>
          <Typography className={styles.question}>
            {i18n.t('components.navbar.modal.didNotReceive')}
          </Typography>
          <Button
          ...
          >
            {i18n.t('components.navbar.modal.resend')}
          </Button>
        </Box>
      </Modal>
      <Box />
